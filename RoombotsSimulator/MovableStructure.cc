...
{
	_position = position - _structure.CenterOffset();
}
...
void MovableStructure::Drop()
{
	...
	_moving = false;
	_position = glm::vec3(_position.x, -EYES_POSITION + 0.01f + _structure.CenterOffset().y, _position.z);
	//_p_button = NULL;
	_buttonID = -1;
...
void MovableStructure::Draw(const glm::mat4& VP) const
{
	...
	//_structure.Draw(VP*glm::translate(glm::mat4(1.0f), _position - _structure.CenterOffset()));
	_structure.Draw(VP*glm::translate(glm::mat4(1.0f), _position));
	if (_moving)
	{
		_shadow.Draw(VP);
	}
