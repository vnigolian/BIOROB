...
_structure(structure), _position(position), _ID(ID), _buttonID(buttonID)
{
	...
	std::cout << "New MovableStructure created with linked button with ID " << _buttonID << std::endl;
}
...
{
	glm::vec3 scaledPosition = (1 / MODULE_SIZE) * (_position);// +_structure.CenterOffset());//CenterPosition();
